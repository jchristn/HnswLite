...
# HnswIndex Server API Test Script
...
# Test 4: GET /v1.0/indexes (List all indexes - should be empty initially)
...
curl -s -X POST "${HEADERS[@]}" \
  ...
  "$BASE_URL/v1.0/indexes/$INDEX_NAME/vectors"
echo ""
echo ""
...
curl -s -X POST "${HEADERS[@]}" \
  ...
  "$BASE_URL/v1.0/indexes/$INDEX_NAME/vectors/batch"
echo ""
echo ""
...
curl -s -X POST "${HEADERS[@]}" \
  ...
  "$BASE_URL/v1.0/indexes/$INDEX_NAME/search"
echo ""
echo ""

# Test 10: GET /v1.0/indexes (List all indexes - should show our created index)
